...
import type { CustomerListQuery, CreateCustomerRequest, UpdateCustomerRequest } from '@erp/contracts/shared';
...
import { getCustomerModel } from './customer-model.js';
...
export class CustomerRepository extends TenantScopedRepository {
  ...
  constructor(connection: mongoose.Connection, private readonly scope: CompanyScope) {
    const model = getCustomerModel(connection);
  ...
  async get(customerId: string, session?: mongoose.ClientSession) {
    return this.customers.findOne(this.filter({ customerId })).session(session ?? null).exec();
  }
  async list(query: CustomerListQuery & { page: number; limit: number }) {
    ...
    const [items, total] = await Promise.all([
      this.customers.find(filter).sort({ createdAt: -1, customerId: 1 }).skip((query.page - 1) * query.limit).limit(query.limit).exec(),
      this.customers.countDocuments(filter).exec(),
    ]);
    return { items, total };
  }
  async insert(input: CreateCustomerRequest, session: mongoose.ClientSession) {
    const rows = await this.customers.create([{
      ...input, tenantId: this.requireTenantId(), companyId: this.scope.companyId,
      customerId: crypto.randomUUID(), status: 'active', version: 1,
      createdBy: this.scope.userId, updatedBy: this.scope.userId,
    }], { session });
    return rows[0]!;
  }
  async patch(customerId: string, input: UpdateCustomerRequest, session: mongoose.ClientSession) {
    const { expectedVersion, ...updates } = input;
    return this.customers.findOneAndUpdate(this.filter({ customerId, version: expectedVersion }), {
