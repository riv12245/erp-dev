...
- Mobile exposes `configureApi(absoluteUrl)` in `apps/mobile/src/services/api-config.ts`; Android emulator default is `http://10.0.2.2:3000`. Physical devices need the development machine's reachable LAN URL. Production must configure the HTTPS API during native startup.
